...
public:
    chatMessage(std::string message, std::string sender, std::string receiver)
    {
...
class ChatBox : public User_Chat
{
private:
    std::string chat_id; // file name
    std::string chat_name;
    std::vector<chatMessage> messages;
    // std::pair<User, User> chatters;

public:
};

class ChatGroup : public User_Chat
{
};
class posts
{
...
public:
    ...
    AVL_Tree<std::string> &get_friends();
};
...
AVL_Tree<std::string> &User ::get_friends()
{
    return friends;
}
...
public:
    ...
    std::unordered_map<std::string, std::list<std::string>> connections;
...
public:
    ...
    int load_main_network_tree(Social_Network &);
...
int DataBase ::load_main_network_tree(Social_Network &sn)
{
    User *u = nullptr;
    ...
    else
    {
        ...
        for (int i = 0; getline(read, data_read, '\n') && i < total_users; i++)
        {
            ...
            u = new User(username, email, name1 + " " + name2, status, interests_code);

            sn.main_tree.insert(sn.main_tree.get_root(), *u);
            load_friends(*u);
            std::cout << "added to tree: " << u->get_username() << std::endl;
        }
...
int main()
{
    ...
    Primary_User *pp = Primary_User::make_primary_user();

    total_users = db->load_main_network_tree(*sn);

    sn->print_tree(sn->main_tree.get_root());
